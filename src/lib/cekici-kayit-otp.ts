...
export interface CekiciKayitOtpKayit {
  telefon: string;
  kod: string;
  olusturulma: string;
  sonGonderim: string;
...
function fromRow(r: CekiciKayitOtpRow): CekiciKayitOtpKayit {
  return {
    telefon: r.telefon,
    kod: r.kod,
...
function toRow(k: CekiciKayitOtpKayit): CekiciKayitOtpRow {
  return {
    telefon: k.telefon,
    kod: k.kod,
...
async function otpGet(telefon: string): Promise<CekiciKayitOtpKayit | undefined> {
  const { data, error } = await getSupabaseAdmin()
    .from("cekici_kayit_otp")
    .select("*")
    .eq("telefon", telefon)
    .maybeSingle();
  if (error) throw error;
  return data ? fromRow(data as CekiciKayitOtpRow) : undefined;
}

async function otpUpsert(kayit: CekiciKayitOtpKayit): Promise<void> {
...
function otpSuresiDolduMu(kayit: CekiciKayitOtpKayit): boolean {
  return Date.now() - new Date(kayit.olusturulma).getTime() > OTP_SURE_DK * 60 * 1000;
}

export async function bekleyenCekiciKayitOtp(telefonHam: string): Promise<{
  bekliyor: boolean;
...
export async function cekiciKayitOtpGonder(
  telefonHam: string
): Promise<
  | {
      ok: true;
...
> {
  ...
  const yeni: CekiciKayitOtpKayit = {
    telefon,
    kod,
    olusturulma:
...
export async function cekiciKayitOtpDogrula(
  telefonHam: string,
  kodHam: string
...
export async function cekiciKayitOtpTemizle(telefonHam: string): Promise<void> {
