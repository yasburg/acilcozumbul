...
export interface CekiciGirisOtpKayit {
  telefon: string;
  kod: string;
  olusturulma: string;
  sonGonderim: string;
...
function fromRow(r: CekiciGirisOtpRow): CekiciGirisOtpKayit {
  return {
    telefon: r.telefon,
    kod: r.kod,
...
function toRow(k: CekiciGirisOtpKayit): CekiciGirisOtpRow {
  return {
    telefon: k.telefon,
    kod: k.kod,
...
async function otpGet(telefon: string): Promise<CekiciGirisOtpKayit | undefined> {
  const { data, error } = await getSupabaseAdmin()
    .from("cekici_giris_otp")
    .select("*")
    .eq("telefon", telefon)
    .maybeSingle();
  if (error) throw error;
  return data ? fromRow(data as CekiciGirisOtpRow) : undefined;
}

async function otpUpsert(kayit: CekiciGirisOtpKayit): Promise<void> {
...
function otpSuresiDolduMu(kayit: CekiciGirisOtpKayit): boolean {
  return Date.now() - new Date(kayit.olusturulma).getTime() > OTP_SURE_DK * 60 * 1000;
}

export async function bekleyenCekiciGirisOtp(telefonHam: string): Promise<{
  bekliyor: boolean;
...
export async function cekiciGirisOtpGonder(
  telefonHam: string
): Promise<
  | {
      ok: true;
...
> {
  ...
  const yeni: CekiciGirisOtpKayit = {
    telefon,
    kod,
    olusturulma:
...
export async function cekiciGirisOtpDogrula(
  telefonHam: string,
  kodHam: string
...
export async function cekiciGirisOtpTemizle(telefonHam: string): Promise<void> {
