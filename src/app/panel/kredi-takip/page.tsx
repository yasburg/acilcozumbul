...
export default function PanelKrediTakipPage() {
  ...
  const yukle = useCallback(async () => {
    ...
    setHata(null);
    try {
      const r = await fetch("/api/panel/kredi-takip");
      if (!r.ok) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h2 className="text-2xl font-bold">Kredi takip SMS</h2>
          <p className="text-sm text-slate-500">
            Kredisi yetmeyen uygun çekicilere hatırlatma — tıklama ve yükleme
            takibi
          </p>
        </div>
      ...
      {ozet && (
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <Card className="bg-slate-50">
            <p className="text-xs text-slate-500">Kredi yükleyen</p>
            <p className="text-2xl font-bold">{ozet.yukleyenCekici}</p>
          </Card>
          <Card className="bg-slate-50">
            <p className="text-xs text-slate-500">Durdurulan (3+ yüksüz)</p>
      ...
      <Card>
        <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
          <h3 className="font-semibold">Manuel gönderim adayları</h3>
        ...
        {!loading && adaylar.length === 0 && (
          <p className="text-sm text-slate-600">
            Şu an gönderilebilir aday yok (kredi yeterli veya 3-kural).
          </p>
        )}
        {!loading && adaylar.length > 0 && (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-slate-500 border-b">
                  ...
                  <th className="py-2 pr-2">Kredi</th>
              ...
              <tbody>
                {adaylar.map((a) => (
                  <tr key={a.cekiciId} className="border-b border-slate-100">
                    ...
                    <td className="py-2 pr-2">{a.kredi}</td>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-slate-500 border-b">
                  ...
                  <th className="py-2 pr-2">Yükleme?</th>
              ...
              <tbody>
                {satirlar.map((s) => (
                  <tr key={s.cekiciId} className="border-b border-slate-100">
                    ...
                    <td className="py-2 pr-2">
                      {s.gonderimSayisi}
                      {s.yuklenmemisBasarili > 0
                        ? ` (${s.yuklenmemisBasarili} yüksüz)`
                        : ""}
                    </td>
                    ...
                    <td className="py-2 pr-2">{s.yukledi ? "Evet" : "Hayır"}</td>
                    <td className="py-2 pr-2">
                      <span
                        className={
                          s.durum === "durduruldu"
