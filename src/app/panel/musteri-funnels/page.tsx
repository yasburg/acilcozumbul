...
type Gunluk = { gun: string; goruldu: number; talep: number };

const FUNNEL_CHIPLER = ["a", "b"] as const;
  ...
  return (
    <div className="rounded-xl bg-slate-100 px-2 py-3 overflow-x-auto">
      <svg
        ...
        role="img"
        aria-label="Müşteri hunisi yatay"
      >
        {adimlar.map((a, i) => {
          const x0 = i * segW;
          const x1 = (i + 1) * segW;
  ...
  return (
    <div className="rounded-xl bg-slate-100 px-2 py-3">
      <svg
        ...
        role="img"
        aria-label="Müşteri hunisi dikey"
      >
        <text
          x={labelW + pctSolW / 2}
          y={12}
...
export default function PanelMusteriFunnelsPage() {
  ...
  const yukle = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/panel/musteri-funnels?${qs}`, {
        credentials: "include",
      });
